...
import pkg from "pdfjs-dist/legacy/build/pdf.js"; // ✅ pure Node PDF render (no pdftoppm)

const pdfjsLib = pkg.default ?? pkg;
const { getDocument } = pdfjsLib;

// Disable worker usage for server environment
...
async function pdfToPNG(pdfPath) {
  ...
  try {
    ...
    await sharp(buffer).resize(designWidth, designHeight).toFile(outPath);
    console.log(`🖼️ Generated PNG (pdfjs) → ${outPath}`);
  } catch (err) {
