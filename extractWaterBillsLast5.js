...
async function pdfToPNG(pdfPath) {
  ...
  const finalPath = path.join(debugDir, `${base}.png`);

  try {
    // 1️⃣ Load PDF via pdfjs-dist
    const data = new Uint8Array(fs.readFileSync(pdfPath));
    const pdfDoc = await pdfjsLib.getDocument({ data }).promise;
    const page = await pdfDoc.getPage(1);

    // 2️⃣ Render PDF page onto canvas
    const viewport = page.getViewport({ scale: 2.5 }); // high DPI render
    ...
    // 4️⃣ Resize and normalize using Sharp
